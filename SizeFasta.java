...
public class SizeFasta {  
  private static final NumberFormat nf = new DecimalFormat("############.#");
   ...
   private static final int MAX_STRING=500000000;
   ...
   public void processFasta(String inputFile) throws Exception {
      ...
      while ((line = bf.readLine()) != null) {
         if (line.startsWith(">")) {
            if (header.length() > 0) {
System.out.println(header + "\t" + (skipNs ? fastaSeq.toString().replaceAll("N", "").length()+len : fastaSeq.length()+len));
if (stopEarly) { System.exit(0); }
            ...
            if ((double)fastaSeq.length() + (double)line.length() >= (double)MAX_STRING) {
               len += skipNs ? fastaSeq.toString().replaceAll("N", "").length() : fastaSeq.length();
               fastaSeq.setLength(0);
            }
...
*/
      ...
      if (fastaSeq.length() != 0) { System.out.println(header + "\t" + (skipNs ? fastaSeq.toString().replaceAll("N", "").length()+len : fastaSeq.length()+len));}
      bf.close();
   }
